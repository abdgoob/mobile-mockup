...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    const canFollowPointer = supportsHover && !reduceMotion;
    ...
    const update = (now: number) => {
      ...
      const allowAutoAnimation = p.autoAnimate && !reduceMotion;
      idleAngle += allowAutoAnimation ? p.speed * dt : 0;
      const target =
        canFollowPointer && p.followMouse && pointerAngle != null && (!allowAutoAnimation || proximityT > 0) ? pointerAngle : idleAngle;
      const diff = ((target - angle + Math.PI * 3) % (Math.PI * 2)) - Math.PI;
      angle += diff * (1 - Math.exp(-dt * 7));

      // Shine fades in with pointer proximity unless autoAnimate keeps it on
      const brightTarget = allowAutoAnimation ? 1 : canFollowPointer ? proximityT : 0;
